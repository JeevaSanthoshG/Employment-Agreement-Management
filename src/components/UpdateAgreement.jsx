...
import { updateAgreement } from "../features/agreements/agreementsSlice";
...
const UpdateAgreement = ({ open, handleClose, agreement }) => {
  ...
  useEffect(() => {
    if (agreement) {
      setValue("employeeName", agreement.employeeName);
      setValue("department", agreement.department);
      setValue("position", agreement.position);
      setValue("agreementDate", agreement.agreementDate);
    }
  }, [agreement, setValue]);
  ...
  const onSubmit = (data) => {
    ...
    dispatch(updateAgreement({ id: agreement.id, ...formattedData }));
    handleClose();
  };
